...
class Calendar
{
  ...
  function wp_calendar ($thismonth, $thisyear, $full)
  {
      ...
      $fstmonthcolor1 = "0047ff";
  		$secntmonthcolor1 = "00b8ff";
    ...
    if ($current->count == 0)
      $initiallink = $month[zeroise($thismonth, 2)].' '.number2bengunicode($thisyear);
    else
  	...
  	/*   if ($thismonth  % 2 == 0) {
  	   $fstmonthcolor = $fstmonthcolor1;
  		$secntmonthcolor = $secntmonthcolor1;
      }
      else {
  	  $fstmonthcolor = $secntmonthcolor1;
  		$secntmonthcolor = $fstmonthcolor1;
  		}      */
      /*if ($thismonth == 4) {
      $initiallink .= ', '. '<font color="#'. $fstmonthcolor . '">'. bongabdoeasy(1, $thismonth, $thisyear,'M', 1) . ' ' . bongabdoeasy(1, $thismonth, $thisyear,'y', 1) . '</font> / <font color="#' .  $secntmonthcolor . '">' . bongabdoeasy(1, $thismonth+1, $thisyear,'M', 1) . '  ' . bongabdoeasy(1, $thismonth+1, $thisyear,'y', 1)  . '</font> ';
      }
      else {
      $initiallink .= ', '. '<font color="#'. $fstmonthcolor . '">'. bongabdoeasy(1, $thismonth, $thisyear,'M', 1) . '</font> / <font color="#' .  $secntmonthcolor . '">' . bongabdoeasy(1, $thismonth+1, $thisyear,'M', 1)  . '</font> ' .  bongabdoeasy(1, $thismonth, $thisyear,'y', 1) ;
      }*/


      if ($thismonth == 4) {
      $initiallink .= ', '. '<font class="bengfirstmonth">'. bongabdoeasy(1, $thismonth, $thisyear,'M', 1) . ' ' . bongabdoeasy(1, $thismonth, $thisyear,'y', 1) . '</font> / <font class="bengsecondmonth">' . bongabdoeasy(1, $thismonth+1, $thisyear,'M', 1) . '  ' . bongabdoeasy(1, $thismonth+1, $thisyear,'y', 1)  . '</font> ';
      }
      else {
      $initiallink .= ', '. '<font class="bengfirstmonth">'. bongabdoeasy(1, $thismonth, $thisyear,'M', 1) . '</font> / <font class="bengsecondmonth">' . bongabdoeasy(1, $thismonth+1, $thisyear,'M', 1)  . '</font> ' .  bongabdoeasy(1, $thismonth, $thisyear,'y', 1) ;
      }


  	echo '<table id="wp-calendar">
  	
  	<caption id="wp-calendar-caption">' . $initiallink; ', ' .
  	'</caption>
  	<thead>
  	<tr>';
	...
	if ($this->showit)
	{
     echo '<td id="showit"><a href="'.get_month_link ($thisyear, $thismonth).'" onclick="return calendar ('.$thisyear.','.$thismonth.','.($full == 0 ? 1 : 0).')">'.$this->split_open.'</a></td>';
	...
	if (count ($this->categories) > 0)
  	...
  	for ( $day = 1; $day <= $daysinmonth; ++$day ) {
  		...
  		if ( $day == gmdate('j', (time() + (get_settings('gmt_offset') * 3600))) && $thismonth == gmdate('m', time()+(get_settings('gmt_offset') * 3600)) && $thisyear == gmdate('Y', time()+(get_settings('gmt_offset') * 3600)) )
  			...
  			echo '<td>';
  			//hacked
echo '<table class = "banglacal"><tr><td class="english">';
  		if ( in_array($day, $daywithpost) ) {// any posts today?
  				//echo '<a href="' . get_day_link($thisyear, $thismonth, $day) . "\" title=\"$ak_titles_for_day[$day]\">$day</a>";
          echo '<a href="' . get_day_link($thisyear, $thismonth, $day) . '" title="';
  				_e($ak_titles_for_day[$day]) ;
  				echo '">'. $day ;
  				echo "</a>";
  			...
  			echo $day;
  		echo '</td></tr>';
  		if (bongabdoeasy($day, $thismonth, $thisyear,'m', 2) == bongabdoeasy(1, $thismonth, $thisyear,'m', 2))
  		  		echo '<tr><td  class="bengfirstmonth">';
  		else echo  '<tr><td  class="bengsecondmonth">';
  		
  		/*main bangla post */
  		if ( in_array($day, $daywithpost) ) {// any posts today? 
  				echo '<a href="' . get_day_link($thisyear, $thismonth, $day) . '" title="';
  				_e($ak_titles_for_day[$day]) ;
  				echo '">'. bongabdoeasy($day, $thismonth, $thisyear,'d', 1) ;
  				echo "</a>";
  				}
  		else
  			echo bongabdoeasy($day, $thismonth, $thisyear,'d', 1);
